...
void exNode( nodeType *p,  int c, int l, /* start column and line of node */  int *ce, int *cm /* resulting end column and mid of node */)
{
	...
	*/
	...
	switch(p->type) 
	{
		...
		case typeId: if(!strcmp(p->id.name,"break")){sprintf (word, "%s", p->id.name);}else{sprintf (word, "id(%s)", p->id.name);} break;
		case typeOpr:
		switch(p->opr.oper)
		{
		...
		//	case UMINUS: s = "[_]"; break;
			...
			case ',': s = "[,]"; break;
			case GE_OP: s = "[>=]"; break;
			case AND_LOG: s = "[&&]"; break;
			case OR_LOG: s = "[||]"; break;
			case LE_OP: s = "[<=]"; break;
			case NE_OP: s = "[!=]"; break;
			case EQ_OP: s = "[==]"; break;
			case 'STRUCT_MEM': s = "[STRUCT MEMBERS]"; break;
			case 'CASES': s = "[CASES]"; break;
			case 'STRUCT': s = "[STRUCT]"; break;
			case 'DEC': s = "[DECLARATION]"; break;
		}
	break;
	}
